...
/**
 * Unit tests for {@link DevTestStopVs}.
 *
 * @author jakro01
 */
public class DevTestStopVsTest extends AbstractDevTestBuildStepTest {
	...
	@Test
	public void testConfigRoundtripWithCredentials() throws Exception {
		FreeStyleProject project = jenkins.createFreeStyleProject();
		project.getBuildersList().add(createPlugin(true, getHost(), "1505", "VSE",
				"file:///C:/webservices-vs.mar\n webservices-vs2.mar\n"
						+ " http://test.com/webservices-vs3.mar", "id")
		);
		project = jenkins.configRoundtrip(project);
		jenkins.assertEqualDataBoundBeans(createPlugin(true, getHost(), "1505", "VSE",
				"file:///C:/webservices-vs.mar\n webservices-vs2.mar\n"
						+ " http://test.com/webservices-vs3.mar", "id"),
				project.getBuildersList().get(0));
	}
	...
	public void testEnvironmentPropertiesInConfig() throws Exception {
		...
		FreeStyleProject project = jenkins.createFreeStyleProject();
		DevTestStopVs devTestStopVs = createPlugin(true, "${host}", "${port}",
				"${vse1}", "${vse2}", "id");
		project.getBuildersList().add(devTestStopVs);

		FreeStyleBuild build = jenkins.assertBuildStatus(Result.FAILURE, project.scheduleBuild2(0));

		jenkins.assertLogContains("Stopping virtual service: test2 on VSE: test", build);
		jenkins.assertLogContains("To DevTest API located on 1.1.1.1:6666", build);
	}

	@Test
	public void testMultipleVsStop() throws Exception {
		vs.useTransaction("stopVs");
	...
	@Test
	public void testMultipleVsStopWithComma() throws Exception {
		vs.useTransaction("stopVs");
	...
	@Test
	public void testStopVs() throws Exception {
		vs.useTransaction("stopVs");
	...
	@Test
	public void testStopNotExistingVs() throws Exception {
		vs.useTransaction("stopVsAndVsDoesntExist");
	...
	@Test
	public void testStopNotExistingVse() throws Exception {
		vs.useTransaction("stopVsAndVseDoesntExist");
	...
	public void testVseNameEmpty() throws Exception {
		...
		DevTestStopVs builder = createPlugin(true, getHost(), "1505", "",
	...
	public void testVseNameNull() throws Exception {
		...
		FreeStyleProject project = jenkins.createFreeStyleProject("test1");
		DevTestStopVs builder = createPlugin(true, getHost(), "1505", null,
				"file:///C:/webservices-vs.mar", "id");
		project.getBuildersList().add(builder);
	...
	public void testVsNameEmpty() throws Exception {
		...
		FreeStyleProject project = jenkins.createFreeStyleProject("test1");
		DevTestStopVs builder = createPlugin(true, getHost(), "1505", "VSE",
				"", "id");
		project.getBuildersList().add(builder);
	...
	public void testScriptedPipeline() throws Exception {
		...
		String pipelineScript
				= "node {\n"
				+ "svStopVirtualService useCustomRegistry: true, vseName: \"VSE\", vsNames: \"webservices-vs\", host:\""
	...
	private DevTestStopVs createPlugin() {
		return createPlugin(true, getHost(), "1505", "VSE", "webservices-vs",
				"id");
	}

	private DevTestStopVs createPlugin(String vseName, String vsNames) {
		return createPlugin(true, getHost(), "1505", vseName, vsNames, "id");
	}
	...
	private DevTestStopVs createPlugin(boolean useCustomRegistry, String host, String port,
		...
		return new DevTestStopVs(useCustomRegistry, host, port, vseName, vsNames, tokenId);
	}
}
