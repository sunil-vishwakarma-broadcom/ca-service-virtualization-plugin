...
public class DevTestUndeployVs extends DefaultBuildStep {
	...
	public void perform(@Nonnull Run<?, ?> run, @Nonnull FilePath workspace,
		...
		for (String vsName : resolvedVsNames) {
			listener.getLogger()
							.println(Messages.DevTestUndeployVs_undeploying(vsName, resolvedVseName));
			listener.getLogger()
			...
			try (CloseableHttpClient client = HttpClients.createDefault();
				...
				if (statusCode == 204) {
					listener.getLogger().println(Messages.DevTestUndeployVs_success(vsName, resolvedVseName));
				} else {
					listener.getLogger()
	...
	@Symbol("svUndeployVirtualService")
	@Extension
	public static final class DescriptorImpl extends DefaultDescriptor {
		 ...
		 * build step for undeploying VS.
		 *
		 * @return name of build step
		 */
		@Override
		public String getDisplayName() {
			return Messages.DevTestUndeployVs_DescriptorImpl_DisplayName();
		}
	}
}
